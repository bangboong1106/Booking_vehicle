...
var Admin = function (parent) {
    ...
    function changeView() {
        var role = $(parent + " input[name='user_role']:checked").val();
        $(parent + " #user_role").val(role);
        $(parent + " #role").val(role);
        ;
        if (role == 'partner') {
            $(parent + " .partner_form").show();
            $(parent + " .admin_form").hide();
            $(parent + " .partner_role").removeAttr('disabled');
            $(parent + " .admin_role").attr('disabled','disabled');
        } else {
            $(parent + " .partner_form").hide();
            $(parent + " .admin_form").show();
            $(parent + " .partner_role").attr('disabled','disabled');
            $(parent + " .admin_role").removeAttr('disabled');
        }
    }

    changeView();
    // event radio change type customer
    $(document).on('click', parent + " input[name='user_role']", function () {
        changeView();
    });
};