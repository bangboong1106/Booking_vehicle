...
function registerSelectDriver(partnerId) {
    ...
    if (typeof is_create !== "undefined") {
        ...
        if (is_create || (!is_create && !is_first_time)) {
            $('.select-driver').val(null).trigger('change');
            $('.select-vehicle-group').val(null).trigger('change');
        }
    ...
    if (typeof cboSelect2 !== "undefined") {
        ...
        if (typeof vehicleGroupDropdownUri !== "undefined") {
            cboSelect2.vehicleGroup(vehicleGroupDropdownUri, '', {partner_id: partnerId});
        }
    }
