...
use App\Repositories\PartnerTemplateRepository;
use Auth;
use Carbon\Carbon;
use Exception;
use Input;
...
class PartnerTemplateController extends BackendController
{

    protected $_fieldsSearch = ['id', 'title'];

    public function __construct(PartnerTemplateRepository $partnerTemplateRepository)
    {
        parent::__construct();
        $this->setRepository($partnerTemplateRepository);
        $this->setBackUrlDefault('partner-template.index');
        $this->setConfirmRoute('partner-template.confirm');
