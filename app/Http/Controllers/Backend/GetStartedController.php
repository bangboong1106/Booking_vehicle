...
class GetStartedController extends BackendController
{
    public function __construct(AppInfoRepository $appInfoRepository) {
        parent::__construct();
        $this->setRepository($appInfoRepository);
        $this->setBackUrlDefault('get-started.index');
        $this->setMenu('get_started');
    ...
    public function redirect($id)
    {
        ...
        return view('backend.get_started.redirect', [
            'id' => $id,
            'data' => $entities
        ]);
    }
    ...
    public function getLinkAppByMail(Request $request)
    {
        ...
        return redirect()->route('get-started.index')->with('success', trans('messages.send_mail_success') . $email);
    }
