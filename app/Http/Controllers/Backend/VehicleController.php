...
class VehicleController extends BackendController
{
    ...
    )
    {
        ...
        $this->setBackUrlDefault('vehicle.index');
        $this->setConfirmRoute('vehicle.confirm');
    ...
    {
        $this->driver_ids = $entity->driver_ids;
    ...
    public function saveDriverVehicle($entity)
    {
    ...
    public function getDistrict()
    {
        ...
        $districtList = empty($provinceId) ? [] : $this->getDistrictRepository()->search($query)->pluck('title', 'district_id');

        $this->setViewData(['districtList' => $districtList]);
        $html = [
            'content' => $this->render('backend.vehicle._district_list')->render()
        ];
    ...
    public function getWards()
    {
        ...
        $wards = empty($districtId) ? [] : $this->getWardRepository()->search($query)->pluck('title', 'ward_id');

        $this->setViewData(['wards' => $wards]);
        $html = [
            'content' => $this->render('backend.vehicle._ward_list')->render()
        ];
    ...
    public function getDataForComboBox()
    {
        ...
        $data = $this->getRepository()->getItemsByUserID($all, $q, $currentUser->id, $partnerId, $currentUser->role);
    ...
    public function getVehicleHistory()
    {
        try {
            $data = Request::all();
            $query = $this->orderHistoryRepository->getCompletedItemsByVehicleID($data);
            ...
            $driver_id_list = $query->where('order_history.primary_driver_id', '!=', 0)
                ->distinct()->pluck('primary_driver_id');
            $driver_query['id_in'] = $driver_id_list->toArray();

            $driver_list = $this->getDriverRepository()->getListForHistory($driver_query);
            ...
            $html = [
                ...
                'drivers_content' => $this->render('backend.vehicle._driver_list_history')->render(),
                'drivers_pagination' => $this->render('backend.vehicle._driver_pagination')->render(),
                'drivers_size' => count($driver_id_list)
            ];
    ...
    public function driverTableAction()
    {
        try {
            $data = Request::all();
            $query = $this->orderHistoryRepository->getCompletedItemsByVehicleID($data);
            ...
            $driver_id_list = $query->where('order_history.primary_driver_id', '!=', 0)
                ->distinct()->pluck('primary_driver_id');
            $data['id_in'] = $driver_id_list->toArray();

            $driver_list = $this->getDriverRepository()->getListForHistory($data);
    ...
    public function getDefaultDriver()
    {
        ...
        $driver_vehicle = $this->driverVehicleRepository->getItemByVehicleID($vehicleId);
        $data = "";
        if ($driver_vehicle != null)
            $data = $this->getDriverRepository()->getItemById($driver_vehicle->driver_id);
        $this->setData($data);
        return $this->renderJson();
    }
    ...
    public function exportTemplate()
    {
        ...
        $vehicleExport = new VehicleExport(
            $this->getRepository(),
        ...
        return $vehicleExport->exportFileTemplate($dataVehicleGroup, $dataDriver, $dataGpsCompany, $partners, $update);
    }

    public function export()
    {
        $vehicleExport = new VehicleExport($this->getRepository(), $this->getDriverVehicleRepository(), $this->getDriverRepository(), $this->_getDataIndex(false));
        return $vehicleExport->exportFromTemplate();
    }

    protected function _mappingDataImport($data, $update)
    {
        $vehicleImport = new VehicleImport();
        ...
        $partnerByVehicleGroups = $this->getVehicleGroupRepository()->search([])->get()->pluck('partner_id', 'code');
        $partnerByDrivers = $this->getDriverRepository()->search([])->get()->pluck('partner_id', 'code');
        $listRegNo = [];
        ...
        if (!empty($data)) {
            foreach ($data as $key => &$row) {
                ...
                $row['group_id'] = isset($vehicleGroups[$row['group_code']]) ? $vehicleGroups[$row['group_code']] : null;
                $row['partner_id'] = isset($partners[$row['partner_code']]) ? $partners[$row['partner_code']] : null;

                if (empty($row['driver_codes'])) continue;
                foreach ($row['driver_codes'] as $driverCode) {
                    if (isset($partnerByDrivers[$driverCode])
                        && $partnerByDrivers[$driverCode] != $row['partner_id']) {
                        $row['importable'] = false;
                        $row['failures'] = ['Tài xế không thuộc đối tác vận tải'];
                    }

                    isset($drivers[$driverCode]) ? $row['driver_ids'][] = $drivers[$driverCode] : null;
                }

                if (isset($partnerByVehicleGroups[$row['group_code']])
                    && $partnerByVehicleGroups[$row['group_code']] != $row['partner_id']) {
                    $item['importable'] = false;
                    $item['failures'] = ['Chủng loại xe không thuộc đối tác vận tải'];
                }
