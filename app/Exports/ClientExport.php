...
class ClientExport implements FromView, ShouldAutoSize, WithColumnFormatting
{
    ...
    public function exportFromTemplate($update = false, $dataCustomer)
    {
        try {
            $index = 1000;
            ...
            foreach ($dataCustomer as $item) {
                $setCell->setCellValue('D' . $rowCustomer, $item->customer_code . '|' . $item->full_name);
                $rowCustomer++;
            }
            ...
            if ($rowCustomer > 1) {
                ...
                $objValidation->setFormula1('Unit!$D$1:$D$' . ($rowCustomer - 1));
                $objPHPExcel->getActiveSheet()->setDataValidation("C10:C" . $index, $objValidation);
            }
            ...
            $objPHPExcel->getActiveSheet()->setDataValidation("D10:D" . $index, $objValidation);

            $objValidation = $objPHPExcel->getActiveSheet()->getCell('M10')->getDataValidation();
            $objValidation->setType(PHPExcel_Cell_DataValidation::TYPE_LIST);
            $objValidation->setShowDropDown(true);
            $objValidation->setFormula1('Unit!$B$3:$B$4');
            $objPHPExcel->getActiveSheet()->setDataValidation("M10:M" . $index, $objValidation);
    ...
    private function addDataToExcelFile($setCell, $data)
    {
        ...
        foreach ($data as $item) {
            $setCell
                ...
                ->setCellValue($cellLocation[$column++] . $row, $item->name_of_parent_id)
