...
class PartnerVehicleExport implements FromView, ShouldAutoSize, WithColumnFormatting
{
    ...
    {
        try {
            $fileTemplatePath = public_path('file/Danh_sach_xe_partner.xlsx');
    ...
    public function exportFileTemplate($dataVehicleGroup, $dataDriver, $dataGpsCompany, $update)
    {
        try {
            ...
            if ($rowDriver > 1) {
                $objValidation2 = $objPHPExcel->getActiveSheet()->getCell('F10')->getDataValidation();
                $objValidation2->setType(PHPExcel_Cell_DataValidation::TYPE_LIST);
                $objValidation2->setShowDropDown(true);
                $objValidation2->setFormula1('Data!$B$1:$B$' . ($rowDriver - 1));
                $objPHPExcel->getActiveSheet()->setDataValidation("F10:F1000", $objValidation2);
            }

            if ($rowGpsCompany > 1) {
                $objValidation3 = $objPHPExcel->getActiveSheet()->getCell('R10')->getDataValidation();
                ...
                $objPHPExcel->getActiveSheet()->setDataValidation("R10:R1000", $objValidation3);
            }
            if ($update) {
                $vehicles = $this->getRepository()->getListForExport($this->_data);
                $this->addDataToExcelFile($objPHPExcel->setActiveSheetIndex(0), $vehicles);
