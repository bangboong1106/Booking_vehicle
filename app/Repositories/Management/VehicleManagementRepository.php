...
class VehicleManagementRepository extends VehicleRepository
{
    ...
    public function getDataListByUser($request, $userId)
    {
        ...
        $columns = [
            ...
            DB::raw(' (drivers.full_name) AS name_of_driver_id ')
        ];
        $items = $query
            ->groupBy(["vehicle.id"])
            ->get($columns);
    ...
    // API lấy thông tin tài xế mặc định của xe
    //Created by nlhoang 28/01/2021
    public function getDefaultDriver($vehicle_id)
    {
        $item = DriverVehicle::join('drivers as d', 'd.id', '=', 'driver_vehicle.driver_id')
