...
class Batch implements BatchInterface
{
        ...
        $ids = [];
            ...
            foreach ($values as $key => $val) {
                ...
                foreach (array_keys($val) as $field) {
                    if ($field !== $index) {
                        $finalField = $raw ? AppConstant::mysql_escape($val[$field]) : "'" . AppConstant::mysql_escape($val[$field]) . "'";
        ...
        foreach ($values as $key => $val) {
            ...
            foreach (array_keys($val) as $field) {
                if ($field !== $index || $field !== $index2 ) {
                    $finalField = $raw ? AppConstant::mysql_escape($val[$field]) : '"' . AppConstant::mysql_escape($val[$field]) . '"';
                    $value = (is_null($val[$field]) ? 'NULL' : $finalField);
                    $final[$field][] = 'WHEN (`' . $index . '` = "' . AppConstant::mysql_escape($val[$index]) .'" AND `'. $index2 . '` = "' . $val[$index2] .'") THEN ' . $value . ' ';
                }
            }
        }
    ...
    public function insert(Model $table, array $columns, array $values, int $batchSize = 500, bool $insertIgnore = false)
    {
        ...
        foreach ($columns as $key => $column) {
            $columns[$key] = '`' . AppConstant::mysql_escape($column) . '`';
        }
        ...
        foreach ($values as $value) {
            ...
            foreach ($value as $data) {
                foreach ($data as $key => $item) {
                    $item = is_null($item) ? 'NULL' : "'" . AppConstant::mysql_escape($item) . "'";
                    $data[$key] = $item;
                }
