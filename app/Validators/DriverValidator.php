...
class DriverValidator extends BaseValidator
{
    ...
    protected function _getMessagesDefault()
    {
        return [
            ...
            '*.code.exists' => trans('validation.exists', ['attribute' => trans('models.driver.attributes.code')]),
            ...
            '*.partner_id.required' => trans('validation.required', ['attribute' => trans('models.driver.attributes.partner_id')]),
            ];
    }
    ...
    public function _buildImportUpdateRules($fromEditor = false)
    {
        $rules = [
            ...
            '*.full_name' => 'required',
            '*.mobile_no' => ['required', 'regex:/(0|\+84)\d{9,10}\b/'],
            '*.code' => 'required|max:50|regex:/^[A-Za-z0-9\-! ,\'\"@\.:\(\)\\\$&#*?~\%\^\+_\/]+$/|distinct|exists:drivers,code',
            '*.vehicle_team_id' => 'nullable|exists:vehicle_team,code',
            '*.birth_date' => 'nullable|date',
            '*.work_date' => 'nullable|date',
            '*.partner_id' => 'required',
