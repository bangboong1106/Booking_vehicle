...
class Base extends Model
{
    ...
    public function getFillable()
    {
        ...
        getSystemConfig('updated_at_column.field') ? $fields[] = getSystemConfig('updated_at_column.field') : null;
        return $fields;
    }
    ...
    public function getAutoIncrementId($numberId = 1)
    {
        ...
        try {
            ...
            $querySelect = "SELECT AUTO_INCREMENT FROM information_schema.TABLES WHERE TABLE_SCHEMA = '" . $schema
                . "' AND TABLE_NAME = '" . $table . "' FOR UPDATE";
            $autoIncrement = DB::selectOne($querySelect);

            if (!$autoIncrement)
                return -1;

            $lastId = $autoIncrement->AUTO_INCREMENT;

            $queryAlter = "ALTER TABLE " . $table . " AUTO_INCREMENT = " . ($lastId + $numberId);
            DB::statement($queryAlter);
    ...
    public function getDate($field = '', $format = 'd-m-Y', $currentFormat = 'Y-m-d')
    {
        if (empty($field)) {
            return '';
        }
        ...
        return Carbon::createFromFormat($currentFormat, $this->$field)->format($format);
    }

    public function getDetailNameField()
    {
        return $this->getAttribute($this->_detailNameField);
    }
}
