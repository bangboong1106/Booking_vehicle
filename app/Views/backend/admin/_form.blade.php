...
<div class="row">
    <div class="col-12">
        {!! MyForm::model($entity, ['route' => ['admin.valid', $entity->id]])!!}
                    ...
                    <div class="col-md-8">
                        <div class="content-body">
                            <div class="form-group row" style="margin-top: 20px">
                                {!! MyForm::hidden('role', $entity->role,['id'=>'role']) !!}
                                <div class="col-md-6">
                                    <div class="radio radio-info form-check-inline">
                                        <input type="radio"
                                               id="user_role_1"
                                               value="admin" name="user_role"
                                                {!! $disabled !!}
                                                {!! !$entity->id || $entity->role == 'admin' ? 'checked' : '' !!}>
                                        <label for="user_role_1"> Tài khoản {{config('constant.APP_NAME')}} </label>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="radio radio-info form-check-inline">
                                        <input type="radio" id="user_role_2" value="partner"
                                               name="user_role"
                                                {!! $disabled !!}
                                                {!! $entity->role == 'partner' ? 'checked' : '' !!}>
                                        <label for="user_role_2"> Tài khoản đối tác vận tải </label>
                                    </div>
                                </div>
                            </div>
                            <div class="row">
                                ...
                                <div class="row" style="padding: 0 8px;">
                                    @foreach($roles as $role)
                                        <div class="custom-control custom-checkbox col-md-4">
                                            <input type="checkbox" class="custom-control-input admin_role" id="role_{{$role->id}}"
                                                   name="listRole[]"
                                                   {!! auth()->user()->can('edit admin') ? '' : 'disabled' !!}
                                                   {!! $entity->hasRole($role->name) || (isset($entity->listRole) && in_array($role->name, $entity->listRole)) ? 'checked' : '' !!}
                                                   value="{{$role->name}}">
                            ...
                            <div class="form-group partner_form">
                                {!! MyForm::label('role', $entity->tA('role')) !!}
                                <div class="row" style="padding: 0 8px;">
                                    @foreach($rolePartners as $role)
                                        <div class="custom-control custom-checkbox col-md-4">
                                            <input type="checkbox" class="custom-control-input partner_role" id="role_{{$role->id}}"
                                                   name="listRole[]"
                                                   {!! auth()->user()->can('edit admin') ? '' : 'disabled' !!}
                                                   checked
                                                   value="{{$role->name}}">
                            ...
                            <div class="form-group row partner_form">
                                <div class="col-md-6">
