@if ($paginator->hasPages())
    <nav>
        <ul class="pagination justify-content-center">
            ...
            @else
                <li onclick="loadOrderPaging({{ $paginator->currentPage() - 1}})" class="page-item"><a
                ...
                @if (is_array($element))
                    @foreach ($element as $page => $url)
                        ...
                        @elseif ($page === $paginator->currentPage() + 1 ||
                            ...
                            $page === 1)
                            <li class="page-item" onclick="loadOrderPaging({{ $page }})"><a class="page-link"
                                                                                            href="#">{{ $page }}</a>
                            </li>
                        @endif
            ...
            @if ($paginator->hasMorePages())
                <li onclick="loadOrderPaging({{ $paginator->currentPage() + 1}})" class="page-item"><a aria-label="Next"
