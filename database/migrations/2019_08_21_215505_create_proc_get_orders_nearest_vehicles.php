...
class CreateProcGetOrdersNearestVehicles extends Migration
{
    /**
     * Run the migrations.
     *
...
BEGIN
    ...
    set @distance = (select CAST(`value` as decimal(18,4)) from system_config where `key` =\'DistanceUnit\' limit 1);
	...
	While exists(Select `vehicle_id` From `tmp_location` Where `valid` = 0) Do
		...
		SELECT 
          ...
          FROM `orders` as o
			join `order_trip` as ot on ot.order_id = o.id
            join `trip` as t on t.id = ot.trip_id
            join `driver_trip` as dt on dt.trip_id = t.id
